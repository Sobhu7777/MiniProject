...
/**
 * Fetch 16-day disaster risk predictions + feature trend data.
 * TODO: Replace with real API call → fetch(`${API_BASE_URL}/forecast-risk?place=${place}`)
 */
export async function fetchForecastRiskData(place) {
    const toLevel = (p) => {
        if (p <= 0.0)  return 'NOT_APPLICABLE';
    ...
    const seed = place.length * 7;

    const data = Array.from({ length: 16 }, (_, i) => {
        const date = new Date();
        date.setDate(date.getDate() + i);
        const dateStr = date.toISOString().split('T')[0];
        const dateFmt = date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });

        // Deterministic but varied probabilities
        const tProb  = Math.min(0.95, Math.max(0, ((seed + i * 13) % 100) / 300));
        const fProb  = Math.min(0.95, Math.max(0, ((seed + i * 7 + 20) % 100) / 200));
        const wProb  = Math.min(0.95, Math.max(0, ((seed + i * 11 + 40) % 100) / 150));
        const lProb  = i < 5 ? 0.0 : Math.min(0.6, ((seed + i * 9 + 10) % 100) / 250);

        // Feature trends — CAPE ramps up sharply near end (per spec)
        const cape          = Math.round(200 + (i * i * 8) + ((seed + i * 3) % 150));
        const precipitation = parseFloat((((seed + i * 5) % 80) / 10 + (i > 10 ? i * 0.5 : 0)).toFixed(1));
        const windGust      = Math.round(15 + ((seed + i * 7) % 30) + (i > 12 ? (i - 12) * 5 : 0));
        const soilMoisture  = parseFloat((0.25 + ((seed + i * 4) % 40) / 100 + (i > 8 ? i * 0.02 : 0)).toFixed(2));
        ...
        return {
            ...
            dateFmt,
            risks: {
                thunderstorm: { level: toLevel(tProb), probability: parseFloat(tProb.toFixed(3)) },
                flood:        { level: toLevel(fProb), probability: parseFloat(fProb.toFixed(3)) },
                windstorm:    { level: toLevel(wProb), probability: parseFloat(wProb.toFixed(3)) },
                landslide:    { level: toLevel(lProb), probability: parseFloat(lProb.toFixed(3)) },
            },
            trends: { cape, precipitation, windGust, soilMoisture },
        };
    });
